...
class DTCoreShortcodesDefination {
		...
		if(!empty($socials)):
			$out .= "<ul class='dt-sc-sociable {$class}'>";
				foreach($socials as $social):
					$out .= "<li> <a class='fa ".$social['icon']."' target='_blank' title='".ucfirst(substr($social['icon'], 3))."' href='".$social['link']."'> </a> </li>";
				endforeach;
			$out .= "</ul>";
		endif;
	...
	function dt_sc_social($attrs, $content = null) {
		...
		foreach ( $sociables as $key => $value ) {
			if(is_array($attrs) && array_key_exists($value, $attrs) && $attrs[$value] != '')
				$s .= '<li><a class="fa '.$key.'" href="'.$attrs[$value].'" title="'.ucfirst($value).'" target="_blank"></a></li>';
		}
